...
public class GetTrashPagedQueryHandlerTests
{
    private readonly Mock<IMailRecipientRepository> _mailRecipientRepo = new();
    private readonly GetTrashPagedQueryHandler _sut;

    public GetTrashPagedQueryHandlerTests()
    {
        _sut = new GetTrashPagedQueryHandler(_mailRecipientRepo.Object);
    }
    ...
    public async Task Handle_ReturnsPagedResult()
    {
        ...
        _mailRecipientRepo.Setup(r => r.GetDeletedPagedAsync(userId, It.IsAny<Domain.Common.Cursor>(), 10, default))
            .ReturnsAsync(mails);

        var result = await _sut.Handle(new GetTrashPagedQuery(userId, pagination), default);
